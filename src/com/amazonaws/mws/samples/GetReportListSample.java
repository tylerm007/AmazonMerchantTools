...
public class GetReportListSample {
    ...
    /**
     * Just add a few required parameters, and try the service Get Report List
     * functionality
     *
     * @param args
     *            unused
	 ...
	 * Service responses without calling Marketplace Web Service service.
	 *
    ...
    /**
     * Get Report List request sample returns a list of reports; by default the
     * most recent ten reports, regardless of their acknowledgement status
     * 
     * @param service
     ...
     *            Action to invoke
     */
    public static void invokeGetReportList(MarketplaceWebService service,
	...
	try {
		...
		System.out.println();
		GetReportListResult getReportListResult = response
		...
		java.util.List<ReportInfo> reportInfoListList = getReportListResult
			.getReportInfoList();
		for (ReportInfo reportInfoList : reportInfoListList) {
			...
			// go get the xml report content
			if( "_GET_FLAT_FILE_ORDERS_DATA".equals(reportInfoList.getReportType()))
					report.getReports(reportInfoList.getReportId());
			System.out.println();
			System.out.print("                    "
